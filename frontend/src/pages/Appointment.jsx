...
const Appointment = () => {
  ...
  return docInfo && (
    ...
    <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
      ...
      <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
        {
          docSlots.length && docSlots.map((item , index) => (
            <div key={index} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-blue-500 text-white' : 'border border-grey-200'}`} onClick={() => {
              setSlotIndex(index);
            }}>
      ...
      <button className='bg-blue-600 text-white text-sm font-light px-14 py-3 rounded-full my-6 cursor-pointer'>Book an Appointment</button>  
    </div>
